...
the theta join or the combination of student and apply on a specific condition***/
...
using clause explicitly lists the attributes that should be equated across the two relations***/
...
/*** Without ON clause 
does not allow you to have both a using clause and an on clause in combination 
with the join, so use where instead of on***/
...
want in our result, the information about students who have not applied anywhere 
left outer join: it takes any tuples on the left side of the join. so any tuples from the
relation that is no the left. and if they do not have a mathcing tuple form the right
andy that is called a dangling tuple. if there is a dangling tuple with no right matching
tuple, it is still added to the result and it is padded with null values.***/
...
/**************************************************************
  FULL OUTER JOIN: want to have unmatched tuples from both the left and 
  the right appear in our result.
...
/**************************************************************
  ...
  Not associative
  left and right outer join is not commutativity 
